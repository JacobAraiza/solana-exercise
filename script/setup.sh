...
alice=$(solana-keygen new -o ./alice-keypair.json --force | grep -o "pubkey: .*" | awk '{print $2}' | tr -d '\n')
...
spl-token mint $token_x 100 $alice_x

bob=$(solana-keygen new -o ./bob-keypair.json --force | grep -o "pubkey: .*" | awk '{print $2}' | tr -d '\n')
#bob_x=$(spl-token create-account $token_x --owner $bob | awk '{print $3}')
bob_y=$(spl-token create-account $token_y --owner $bob | awk '{print $3}')
spl-token mint $token_y 100 $bob_y
